...
def date_handler(obj):
    return datetime.datetime.strftime(obj, "%Y/%m/%d") if hasattr(obj,
                                                                  'isoformat') else obj


def datetime_handler(obj):
    return datetime.datetime.strftime(obj, "%Y/%m/%d %H:%M") if hasattr(
        obj, 'isoformat') else obj


def response_json(content, status_code, parse_date=True, parse_datetime=False):
    """
    ...
    @param parse_date: value to parse date
    @type parse_date: bool
...
def paginate_results(results, base_url, search_term, results_per_page,
    ...
    # TODO: Fix page range and pagination

    first_page = pr[0][0]
    last_page = pr[len(pr) - 1][0]
...
def add_months(sourcedate, months):
    """ Adds n number of months to a date
...
class UnexpectedType(Exception, ):
    def __init__(self, expected, received_type):
        """ Custom exception for custom detail object initializers

        :param expected: Name of the expected class
        :type expected: str
        :param received_type: Name of the unexpected_type
        :type received_type: str
        """
        message = "Unexpected type, {0} or int expected, " \
                  "{1} received".format(expected, received_type)
...
class WorkOrderValidation(Exception, ):
    def __init__(self, value_or_pk, lookup_field, model):
        """ Custom exception for custom form validations

        :param value_or_pk: received value
        :type value_or_pk: str
        :param model: Name of the unexpected_type
        :type model: class
        """
        message = "No se encontró un {0} con {1} = " \
                  "{2}".format(value_or_pk, lookup_field, model)
        # Call the base class constructor with the parameters it needs
        Exception.__init__(self, message)